...
use sarvam::{
    types::{
        ...
        SpeechBatchUploadUrlsRequest, SpeechToTextBatchJobParameters, SttMode,
    },
    SarvamClient,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let started = client.speech_to_text_batch().start_job(&job.job_id).await?;
    ...
    if status.job_state.is_success_like() {
        ...
        let downloads = client
            .speech_to_text_batch()
            .get_download_urls(SpeechBatchDownloadUrlsRequest {
                job_id: job.job_id.clone(),
                files: output_files,
            })
            .await?;
